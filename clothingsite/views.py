...
def men_sweetshirt(request):
    ordering = request.GET.get('ordering', '')
    price = request.GET.get('price', '')
    men_sweatshirt_products = Product.objects.filter(subcategory = 'men-sweatshirt')

    if ordering:
        men_sweetshirt_products = men_sweetshirt_products.order_by(ordering)

    if price:
        men_sweetshirt_products = men_sweetshirt_products.filter(price__lt = price)
...
def women_suit(request):
    ordering = request.GET.get('ordering', '')
    price = request.GET.get('price', '')
    women_suit_products = Product.objects.filter(subcategory = 'women-suit')

    if ordering:
        women_suit_products = women_suit_products.order_by(ordering)

    if price:
        women_suit_products = women_suit_products.filter(price__lt = price)

    return render(request,'clothingsite/women_suit.html', {'women_suit_products' : women_suit_products})
